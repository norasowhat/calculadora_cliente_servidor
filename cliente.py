...
class Cliente:
    ...
    def enviar_operacion(self):
        ...
        try:
            self.sock.sendall(json.dumps(diccionario_cliente).encode('utf-8'))
            self.texto_pantalla.set("")
        except Exception as e:
            messagebox.showerror("Error")
    ...
    def escuchar_respuestas(self):
        while True:
            try:
                resultado = self.sock.recv(1024).decode('utf-8')
                if not resultado:
                    break
                
                mensaje = json.loads(resultado)
                if mensaje.get("tipo") == "respuesta":
                    linea = f"[Serv. {mensaje['id_servidor']}] {mensaje['expresion']} = {mensaje['resultado']}\n"
